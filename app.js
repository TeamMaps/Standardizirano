...
.get(function(req,res,next){
    ...
    prijatelji.push(req._passport.session.user.facebook_id);
     connection.query('SELECT username,photo,title,opis,lat,lng,privacy,marker_id,path,icon FROM Users JOIN Markers ON user_id = Users_user_id WHERE facebook_id IN ('+ prijatelji.join()+ ') || privacy = 0 ORDER BY privacy DESC,marker_id DESC LIMIT 250', function(err, rows){ 
        if (err) throw err;
        res.json(rows);
    })
    }
    else connection.query('SELECT username,photo,title,opis,lat,lng,privacy,marker_id,path,icon FROM Users JOIN Markers  ON Users_user_id = user_id WHERE privacy = 0 ORDER BY marker_id DESC LIMIT 250',function(err,rows){
       if(err) throw err;
       res.json(rows);
   })
})
